...
#endif
extern opl_driver_t opl_epub_driver;
...
void OPL_Delay(uint64_t us)
{
    ...
    // EPUB build: single-threaded, so advance the software emulator
    // synchronously instead of blocking for an audio callback that can
    // never run while the main thread is waiting.

    OPL_Epub_AdvanceTime(us);
}
